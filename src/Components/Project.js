...
const Project = ({ key, title, image, description, techstack, previewLink, githubLink }) => {
  const desc = useRef(null);
  const showAllText = (id) => {
  ...
  return (
    ...
    //   </div>
    // </article>
    <div className="overflow-hidden rounded my-5 bg-white text-slate-500 shadow-md shadow-slate-200">
      <figure>
        <img
          src={image} alt={title} className="aspect-video w-full"
        />
      </figure>
      <div className="p-6">
        <header className="mb-4">
          ...
          <p className="text-sm text-slate-400"> {techstack} 
          </p>
        </header>
        <p ref={desc} id={title.replace(/\s/g, "")} className="truncate text-wrap">
          {description}
        </p>
        <button type="button" className="text-sm text-red-500" onClick={showAllText(title.replace(/\s/g, ""))}>Read more</button>
      </div>
      <div className="flex justify-between items-center px-4 pt-0 pb-2">
        <div className="flex items-center">
          <svg
            className="stroke-dark-heading dark:stroke-white inline-block min-w-fit"
            width="20"
